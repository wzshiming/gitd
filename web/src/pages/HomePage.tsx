...
import { fetchRepos, createRepository, deleteRepository, importRepository, getImportStatus, syncRepository } from '../api/client';
...
export function HomePage() {
  ...
  const handleCreateRepository = async () => {
    ...
    setActionError(null);
    try {
      const repoName = newRepoName.endsWith('.git') ? newRepoName : `${newRepoName}.git`;
  ...
  const handleSyncRepository = async (name: string) => {
    ...
    try {
      ...
      const pollStatus = async () => {
        try {
          const status = await getImportStatus(repoName);
  ...
  const handleImportRepository = async () => {
    ...
    setActionError(null);
    try {
      const repoName = importRepoName.endsWith('.git') ? importRepoName : `${importRepoName}.git`;
